...
class NeuralMemory:
    ...
    def ingest(self, content, metadata=None):
        ...
        print(f"[NMS] Ingesting: {content[:VAR_30]}...")
        
        embedding = self._get_embedding(content)
        
        memory_shard = {
            "id": f"mem_{int(time.time()*VAR_1000)}",
            "content": content,
    ...
    def recall(self, query, limit=VAR_5, threshold=VAR_0_6):
            ...
            if query_vec:
                try:
                    ...
                    for shard in self.memory_index:
                        if shard.get('embedding'):
                            all_embeddings.append(shard['embedding'])
                            valid_shards.append(shard)
                    
                    if all_embeddings:
                        # Convert to tensors
                        q_tensor = torch.tensor(query_vec, device=device).unsqueeze(0) # [1, D]
                        m_tensor = torch.tensor(all_embeddings, device=device)        # [N, D]
                        
                        # Normalize for cosine similarity
                        q_norm = q_tensor / q_tensor.norm(dim=1, keepdim=True)
                        m_norm = m_tensor / m_tensor.norm(dim=1, keepdim=True)
                        
                        # Batch calculate: [1, D] * [D, N] = [1, N]
                        similarities = torch.mm(q_norm, m_norm.t()).squeeze(0)
                    ...
                    print(f"[NMS] GPU Semantic Search failed: {e}. Falling back to standard method.")
                    # Standard method (simplified fallback)
                    for shard in self.memory_index:
                        if not shard.get('embedding'): continue
