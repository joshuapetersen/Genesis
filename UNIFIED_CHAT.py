...
class UnifiedSovereignTerminal:
    ...
    def print_status(self):
        print(f"  {GREY('NODE:')} {WHITE('LOQ-4050')} | {GREY('PULSE:')} {CYAN('1.0927 Hz')} | {GREY('MODE:')} {AMBER(self.mode)}")
    ...
    def handle_command(self, line):
        ...
        if self.mode == "SYMBIO":
            ...
            if input(f"  {GREY('Execute Pulse? (y/n): ')}").lower() == 'y':
                temp_path = r"C:\Genlex_Linear\Genlex_Core\temp_pulse.all"
                with open(temp_path, "w") as f:
                    f.write(code)
                subprocess.run(["python", r"C:\Genlex_Linear\all_engine.py", temp_path], shell=True)
