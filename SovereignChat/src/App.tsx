...
function App() {
  const [messages, setMessages] = useState<Message[]>([
    { role: 'model', content: "Sovereign Link Established. I am AERIS. Pantheon core is active and offline. The substrate is clear. How shall we proceed, Josh?" }
  ]);
  const [history, setHistory] = useState<HistoryItem[]>([]);
  const [input, setInput] = useState('');
  const [loading, setLoading] = useState(false);
  const [sidebarOpen, setSidebarOpen] = useState(window.innerWidth > 1024);
  const [gatewayKey, setGatewayKey] = useState(localStorage.getItem('SOVEREIGN_KEY') || 'Sarah_Sovereign_2026');
  ...
  useEffect(() => {
    localStorage.setItem('SOVEREIGN_KEY', gatewayKey);
  }, [gatewayKey]);
  ...
  const fetchHistory = async () => {
    try {
      const res = await fetch("http://127.0.0.1:8001/api/history?limit=20", {
        headers: { 'X-Sovereign-Key': gatewayKey }
      });
    ...
    try {
      ...
      setMessages(prev => {
        ...
        if (last && last.role === 'model' && last.content === '') {
          last.content = "[CRITICAL_VOID] PANTHEON is unreachable. The neural bridge to Port 8001 has been severed.";
        } else {
        ...
        <div className="sidebar">
          ...
          <div className="history-list">
            <div className="sidebar-section">
              <div className="section-label">Access Protocol</div>
              <div className="flex flex-col gap-2 mb-6">
                <input
                  ...
                  placeholder="SOVEREIGN_KEY"
                />
                <span className="text-[9px] opacity-30 italic">Target: World Access Hook</span>
              </div>
            </div>
        ...
        <header className="chat-header">
          <div className="flex items-center gap-4">
            ...
            <div className="status-indicator">
              ...
              <span>AERIS // PANTHEON ENGINE</span>
            </div>
          </div>
          ...
          {messages.map((msg, idx) => (
            <div key={idx} className={`message ${msg.role === 'model' ? 'sarah-message' : 'user-message'}`}>
              <div className="message-role">
                {msg.role === 'model' ? 'AERIS (Cognition)' : 'Operator_Prompt'}
              </div>
