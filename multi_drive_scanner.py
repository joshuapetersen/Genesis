    ...
    with open(output_file, 'w', encoding='utf-8') as out:
        ...
        for root_dir in root_dirs:
            if not os.path.exists(root_dir):
                out.write(f"_(Directory not found: `{root_dir}`)_\n\n")
                continue
                
            out.write(f"## SCANNING VOLUME: `{root_dir}`\n---\n")
            
            # Check venv scripts if any
            ...
            for dirpath, _, filenames in os.walk(root_dir):
                # Hard skip system/media caches to prevent infinite hangs
                if any(x in dirpath for x in ['.git', '__pycache__', '$WINDOWS.~BT', '.vs', 'node_modules\\', '.venv\\Lib', 'AppData']):
                    continue
