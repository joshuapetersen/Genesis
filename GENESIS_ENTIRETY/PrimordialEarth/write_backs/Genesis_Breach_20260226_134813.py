...
def main_loop():
    ...
    while True:
        ...
        sync_tick(tick)  # Live sync to Supabase (background thread, non-blocking)
        ...
        for row in rows:
            ...
            # Count nearby souls within 300 units — community = survival
            neighbors = sum(1 for t_id, (tx, ty, _, _, _, _) in pos_map.items()
                            if t_id != sid and math.sqrt((x-tx)**2 + (y-ty)**2) < 300)
            if neighbors >= 10:
                new_e += 1.5      # City bonus — thriving settlement
                if tick % 500 == 0 and random.random() < 0.05:
                    print(f"  [CIVILIZATION] City near ({int(x)},{int(y)}) — {neighbors} souls thriving.")
            elif neighbors >= 5:
                new_e += 0.8      # Village bonus
            elif neighbors >= 2:
                new_e += 0.3      # Community bonus — even 2 is better than none
            ...
            # A soul alone in the wilderness has a harder road — but a higher ceiling.
            # Settlers gain stability. Wanderers gain potential.
            if neighbors == 0:
                # Every tick alone builds endurance
                new_e += 0.4  # Lean survival — they learn to need less
                ...
                if ws > 45 and random.random() < 0.001:
                    cur.execute("UPDATE souls SET blessing = 'Wanderer\\'s Crown', wis = min(wis+2,99) WHERE soul_id=?", (sid,))
            ...
            if next_action != action or (bless == "Sovereign Anchor" and random.random() < 0.3):
            ...
            # Protection: If Sovereign Anchor is active, skip the standard "cannot take this" overwrite
            # to allow the Architect's whisper to persist for the first few ticks.
            if bless == "Sovereign Anchor" and "DIVINE:" in (hope or ""):
                pass # Allow the whisper to persist
            else:
