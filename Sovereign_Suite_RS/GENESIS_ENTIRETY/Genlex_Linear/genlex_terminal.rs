...
pub fn c(t: &str, r: &str, g: &str, b: &str) {
        ...
        CYAN = |t | {  c ( t , 0 , 255 , 204 ) };
        DCYAN = |t | {  c ( t , 0 , 120 , 120 ) };
        WHITE = |t | {  c ( t , 220 , 220 , 220 ) };
        GREEN = |t | {  c ( t , 0 , 255 , 100 ) };
        RED = |t | {  c ( t , 255 , 80 , 80 ) };
        AMBER = |t | {  c ( t , 255 , 180 , 0 ) };
        GREY = |t | {  c ( t , 80 , 80 , 80 ) };
        PINK = |t | {  c ( t , 200 , 80 , 200 ) };
        INTENTS = [;
        ( [ "list files" , "show files" , "ls" , "dir" ] , "LIST_FS" ,;
        |args | {  format!("Get-ChildItem {args if args && !args.lower() in ('files','file','') else '.'} | Format-Table Name,Length,LastWriteTime -Auto" ) , });
        ( [ "read file" , "read" , "open file" , "cat" , "show file" ] , "READ_FS" ,;
        |args | {  format!("Get-Content \"{args}\"" if args else "Write-Host 'Usage: read <filename>'" ) , });
        ( [ "delete file" , "remove file" , "delete" , "remove" , "rm" ] , "DELETE_FS" ,;
        |args | {  format!("Remove-Item \"{args}\" -Confirm" if args else "Write-Host 'Usage: delete <filename>'" ) , });
        ( [ "make dir" , "new folder" , "mkdir" , "create dir" ] , "MKDIR" ,;
        |args | {  format!("New-Item -ItemType Directory -Path \"{args}\"" if args else "Write-Host 'Usage: mkdir <name>'" ) , });
        ( [ "move file" , "move" , "mv" ] , "MOVE_FS" ,;
        |args | {  format!("Move-Item {args}" if args else "Write-Host 'Usage: move <src> <dst>'" ) , });
        ( [ "copy file" , "copy" , "cp" ] , "COPY_FS" ,;
        |args | {  format!("Copy-Item {args}" if args else "Write-Host 'Usage: copy <src> <dst>'" ) , });
        ( [ "process list" , "list processes" , "processes" , "tasks" , "running" ] , "LIST_PROC" ,;
        |args | {  "Get-Process | Sort-Object CPU -Descending | Select-Object -First 20 | Format-Table Name,CPU,WorkingSet -Auto" ) , };
        ( [ "kill process" , "kill" , "stop process" , "stop" , "terminate" ] , "KILL_PROC" ,;
        |args | {  format!("Stop-Process -Name {args} -Force" if args else "Write-Host 'Usage: kill <processname>'" ) , });
        ( [ "launch" , "start app" , "open app" , "run app" ] , "LAUNCH_APP" ,;
        |args | {  format!("Start-Process {args}" if args else "Write-Host 'Usage: launch <app>'" ) , });
        ( [ "ping" ] , "PING" ,;
        |args | {  format!("ping {args if args else '8.8.8.8'} -n 4" ) , });
        ( [ "network info" , "ip address" , "network" , "interfaces" , "ipconfig" ] , "NET_INFO" ,;
        |args | {  "Get-NetIPAddress | Format-Table InterfaceAlias,IPAddress -Auto" ) , };
        ( [ "wifi" , "wireless" ] , "WIFI_STATUS" ,;
        |args | {  "netsh wlan show interfaces" ) , };
        ( [ "free memory" , "memory usage" , "memory" , "ram" ] , "MEM_STATUS" ,;
        |args | {  "(Get-WmiObject Win32_OperatingSystem | Select-Object @{n='FreeGB';e={[math]::Round($_.FreePhysicalMemory/1MB,2)}},@{n='TotalGB';e={[math]::Round($_.TotalVisibleMemorySize/1MB,2)}} | Format-List)" ) , };
        ( [ "disk space" , "disk usage" , "disk" , "storage" , "drives" ] , "DISK_STATUS" ,;
        |args | {  "Get-PSDrive -PSProvider FileSystem | Format-Table Name,@{n='UsedGB';e={[math]::Round($_.Used/1GB,1)}},@{n='FreeGB';e={[math]::Round($_.Free/1GB,1)}},Root -Auto" ) , };
        ( [ "cpu usage" , "cpu load" , "cpu" , "processor" ] , "CPU_STATUS" ,;
        |args | {  "Get-WmiObject Win32_Processor | Select-Object Name,LoadPercentage | Format-List" ) , };
        ( [ "system info" , "sysinfo" , "system" , "info" ] , "SYS_INFO" ,;
        |args | {  "Get-ComputerInfo | Select-Object CsName,OsName,WindowsVersion | Format-List" ) , };
        ( [ "time" , "clock" , "date" ] , "SYS_TIME" ,;
        |args | {  "Get-Date" ) , };
        ( [ "uptime" ] , "UPTIME" ,;
        |args | {  ""Uptime: " + ((Get-Date) - (gcim Win32_OperatingSystem).LastBootUpTime).ToString()" ) , };
        ( [ "run genlex" , "execute genlex" , "genlex run" , "run script" ] , "EXEC_GENLEX" ,;
        |args | {  format!("python C:\\Genlex_Linear\\all_engine.py {args}" if args else "Write-Host 'Usage: run genlex <file.all>'" ) , });
        ...
        lex = { };
        // try {
        // with scope: open ( GENLEX_MAP , "r" , encoding = "utf-8" ) as f  {
        ...
        // try {
        // with scope: open ( HISTORY_FILE , "w" , encoding = "utf-8" ) as f  {
        json . dump ( hist [ -200 : ] , f , ensure_ascii = false , indent = 2 );
        // } catch  Exception  {
        // pass
        pub fn load_history ( )  {
        // try {
        // with scope: open ( HISTORY_FILE , "r" , encoding = "utf-8" ) as f  {
        return  json . load ( f );
        // } catch  Exception  {
        return  [ ];
        pub fn main ( )  {
        ...
        while true  {
        // try {
        prompt = ( format!("\033[38;2;0;80;80m╔[\033[38;2;0;200;150mGENESIS\033[38;2;0;80;80m]");
        format!("─[\033[38;2;0;120;100m{os.path.basename(cwd)}\033[38;2;0;80;80m]\033[0m\n");
        format!("\033[38;2;0;80;80m╚▶ \033[0m" ));
        ...
        // with scope: open ( r "C:\Genlex_Linear\execution_seal.json" , "w" ) as f  {
