...
impl SyntaxErrorDetector {
    pub fn new() -> Self {
        ...
        error = CodeError (;
        error_id = format!("SYNTAX_{file_path}_{e.lineno}" ,);
        ...
        error = CodeError (;
        error_id = format!("FILE_NOT_FOUND_{file_path}" ,);
        severity = ErrorSeverity . CRITICAL ,;
        error_type = "FILE_ERROR" ,;
        file_path = file_path ,;
        message = format!("File !found: {file_path}" ,);
        suggestion = "Verify file path == correct";
        );
        errors . append ( error );
        ...
        context . append ( format!("{marker}{i+1}: {lines[i]}" ));
        return  "\n" . join ( context );
    }
