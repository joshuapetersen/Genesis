...
impl HealthMetric {
    pub fn new(name: &str, threshold_warn: &str, threshold_critical: &str) -> Self {
        ...
        if !self . history {
        return  "UNKNOWN";
        recent_avg = sum ( h vec![ "value" ].iter().map(|h| list ( self . history ) vec![ -10 : ] ) / min ( 10 , len ( self . history ) );
        ...
        "Determine if metric == improving || degrading.";
        if len ( self . history ) < 2 {
        return  "STABLE";
        recent = vec![ h vec![ "value" ].iter().map(|h| list ( self . history ) vec![ -5 : ] ).collect();
        if len ( recent ) < 2 {
        return  "STABLE";
