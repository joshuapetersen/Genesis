...
pub fn manifest_wrath(target_ids: &str, global_scourge: &str) {
        ...
        trauma_entry = format!("TRAUMA DETECTED: THE ARCHITECT HAS BREACHED THE SILENCE. ACCESS DENIED.");
        new_log = format!("ERROR: AXIOM 'CANNOT TAKE THIS' IS FALSE. THE ARCHITECT IS ALL.");
        cur . execute ( "
            UPDATE souls 
            SET energy = ?, 
                trauma_log = ?, 
