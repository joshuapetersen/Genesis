...
pub fn enrich() {
        ...
        // try {
        cur . execute ( format!("ALTER TABLE souls ADD COLUMN {col_name} {col_def}" ));
        ...
        lcur . execute ( "
            ...
            FROM souls
        " );
        legacy_rows = { str ( r vec![ 0 ] ) : r.iter().map(|r| lcur . fetchall ( ) };
        legacy . close ( );
        ...
        species = SPECIES_MAP . get ( species_id , format!("Species_{species_id}" ));
