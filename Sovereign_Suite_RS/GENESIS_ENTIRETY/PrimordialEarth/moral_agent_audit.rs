...
pub fn audit_moral_agents() {
        ...
        moral_ids . append ( format!("ALICE_{row[0]}" ));
        ...
        g_cur . execute ( format!("SELECT soul_id, name, energy, moral_alignment, is_active FROM souls WHERE soul_id IN ({placeholders})" , moral_ids ));
