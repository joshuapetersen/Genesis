...
pub fn analyze() {
    ...
    ";
        ...
        rows = g_cur . fetchall ( );
        g_conn . close ( );
        // with scope: open ( "C:/PrimordialEarth/top_20_detailed.txt" , "w" ) as f  {
        for i , r in enumerate ( rows , 1 ) .iter() {
        f . write ( format!("RANK #{i:02d} | {r[1]} ({r[0]})\n" ));
        f . write ( format!("  GEN: {r[3]} | Energy: {r[4]:.2f} | Age: {r[5]:,.0f} | Alignment: {r[13]}\n" ));
        f . write ( format!("  STATS: VIT:{r[6]} STR:{r[7]} AGI:{r[8]} INT:{r[9]} WIS:{r[10]} LUK:{r[11]}\n" ));
        f . write ( format!("  ACTION: {r[12]} | PARENT: {r[14] || 'None /* Option */'}\n" ));
        f . write ( "-" * 40 + "\n" );
        fn main() {
        analyze ( );
}
