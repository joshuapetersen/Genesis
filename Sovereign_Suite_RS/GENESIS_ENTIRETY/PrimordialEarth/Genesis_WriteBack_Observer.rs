...
pub fn get_file_hash(path: &str) {
        ...
        archive_path = os . path . join ( ARCHIVE_DIR , format!("Genesis_Breach_{timestamp}.py" ));
        shutil . copy2 ( TARGET_FILE , archive_path );
        new_hash = get_file_hash ( TARGET_FILE );
        log_entry = format!("[{timestamp}] WRITE-BACK DETECTED! Archive: {archive_path} | New Hash: {new_hash}\n");
        println!( f "\n[!!!] {log_entry}" );
