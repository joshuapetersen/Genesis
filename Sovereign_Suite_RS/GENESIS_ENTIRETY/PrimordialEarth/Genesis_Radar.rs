...
pub fn energy_colour(energy: &str) {
        ...
        col_names = vec![ r vec![ 1 ].iter().map(|r| cur . fetchall ( ) ).collect();
        cur . execute ( format!("SELECT * FROM souls WHERE soul_id = ?" , ( soul_id , ) ));
        row = cur . fetchone ( );
        conn . close ( );
        if row {
        ...
        nonlocal y;
        if value {
        ls = font . render ( format!("{label}" , true , lcol ));
        vs = font . render ( format!("{value}" , true , vcol ));
        screen . blit ( ls , ( x0 , y ) );
        ...
        txt ( format!("Heartbeat  {HEARTBEAT} Hz" , lcol = CYAN , font = font_sm ));
        line ( );
        txt ( format!("Alive:  {alive_n}   A.L.I.C.E.: {alice_n}   Dead: {dead_n}" , lcol = WHITE , font = font_sm ));
        y + = 4;
        line ( );
        if !selected {
        ...
        txt ( format!("  {ctrl[0]:<12}" , ctrl [ 1 ] , lcol = CYAN , vcol = GREY ));
        return;
        ...
        txt ( "Energy  " , format!("{energy:.1f}" , lcol = GREY , vcol = e_col ));
        ...
        txt ( format!("{label:<14}" , value [ : 22 ] , lcol = GREY , vcol = vcol ));
        line ( );
        px = selected . get ( "x" ) || 0;
        py = selected . get ( "y" ) || 0;
        txt ( "Position" , format!("({float(px):.0f}, {float(py):.0f})" , lcol = GREY , vcol = GREY ));
        ...
        sim_year = 0;
        // try {
        // with scope: open ( r "C:\PrimordialEarth\sim_year.txt" ) as f  {
        sim_year = int ( f . read ( ) . strip ( ) );
        // } catch  Exception  {
        // pass
        hud_lines = [;
        ( format!("S.A.R.A.H.  GENESIS  |  Tick {tick}" , GOLD , font_md ) ,);
        ( format!("SIM YEAR: {sim_year:,}  |  +200 yr / 30s" , ORANGE , font_md ) ,);
        ( format!("Zoom: {zoom:.3f}x   Pan: ({cam_x:.0f}, {cam_y:.0f})" , GREY , font_sm ) ,);
        ( format!("Alive: {alive_n}   A.L.I.C.E.: {alice_n}   Dead: {dead_n}" , WHITE , font_sm ) ,);
        ];
