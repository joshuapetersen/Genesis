...
pub fn audit_morality() {
    ...
    " );
        rows = cur . fetchall ( );
        report_lines = [ format!("--- MORAL HARMONY AUDIT (Population: 3,670) ---\n" ]);
        for soul_id , hope , mandate , wis , int_ in rows .iter() {
        text = format!("{hope || ''} {mandate || ''}" . lower ( ));
        findings = [ ];
        for category , keywords in ETHICAL_AXIOMS . items ( ) .iter() {
        matches = vec![ kw.iter().map(|kw| keywords if kw| text ).collect();
        if matches {
        findings . append ( format!("{category}({len(matches)})" ));
        if findings {
        axiom_summary = ", " . join ( findings );
        if ( wis || 0 ) > 50 {
        report_lines . append ( format!("[*] SOVEREIGN AGENT {soul_id} (WIS: {wis}) | {axiom_summary}\n" ));
        report_lines . append ( format!("    EXTRACTED AXIOM: {mandate[:200] if mandate else 'No active philosophy'}\n" ));
        if "destructive" in str ( findings ) . lower ( ) {
        report_lines . append ( format!("    [!] WARNING: Potential Moral Divergence detected.\n" ));
