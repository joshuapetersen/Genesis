//! show-only_json-v1_check.py (Rust Edition)
//! Auto-transpiled by Sovereign First-Principles Engine
//! Axiom: 1.09277703703 Hz

// use std::env;
// use crate::show_only_json_check::{};

pub fn check_kind(k: &str) {
        assert is_string ( k );
        assert k == "ctestInfo";
        pub fn check_version ( v ) {
        assert is_dict ( v );
        ...
        pub fn check_backtracegraph_commands ( c ) {
        assert is_list ( c );
        assert len ( c ) == 1;
        assert is_string ( c [ 0 ] );
        assert c [ 0 ] == "add_test";
        pub fn check_backtracegraph_files ( f ) {
        assert is_list ( f );
        assert len ( f ) == 2;
        ...
        assert f [ 0 ] == "file1";
        assert f [ 1 ] == "file0";
        pub fn check_backtracegraph_nodes ( n ) {
        assert is_list ( n );
        assert len ( n ) == 2;
        node = n [ 0 ];
        assert is_dict ( node );
        ...
        assert c [ 2 ] == "echo";
        pub fn check_generated_resource_spec_file_property ( p ) {
        assert is_dict ( p );
        ...
        pub fn check_reqfiles_property ( p ) {
        assert is_dict ( p );
        assert sorted ( p . keys ( ) ) == [ "name" , "value" ];
        assert is_string ( p [ "name" ] );
        ...
        pub fn check_timeout_property ( p ) {
        assert is_dict ( p );
        assert sorted ( p . keys ( ) ) == [ "name" , "value" ];
        assert is_string ( p [ "name" ] );
        ...
        pub fn check_timeout_signal_name_property ( p ) {
        assert is_dict ( p );
        assert sorted ( p . keys ( ) ) == [ "name" , "value" ];
        assert is_string ( p [ "name" ] );
        assert is_string ( p [ "value" ] );
        ...
        pub fn check_timeout_signal_grace_property ( p ) {
        assert is_dict ( p );
        assert sorted ( p . keys ( ) ) == [ "name" , "value" ];
        assert is_string ( p [ "name" ] );
        assert is_float ( p [ "value" ] );
        ...
        assert p [ "value" ] == 2.1;
        pub fn check_willfail_property ( p ) {
        assert is_dict ( p );
        assert sorted ( p . keys ( ) ) == [ "name" , "value" ];
        assert is_string ( p [ "name" ] );
        ...
        assert p [ "value" ] == true;
        pub fn check_resource_groups_property ( p ) {
        assert is_dict ( p );
        assert sorted ( p . keys ( ) ) == [ "name" , "value" ];
        assert is_string ( p [ "name" ] );
        assert is_list ( p [ "value" ] );
        ...
        pub fn check_workingdir_property ( p ) {
        assert is_dict ( p );
        assert sorted ( p . keys ( ) ) == [ "name" , "value" ];
        assert is_string ( p [ "name" ] );
        assert is_string ( p [ "value" ] );
        ...
        pub fn check_defined_properties ( p_list ) {
        for property_id , p in zip ( [ "A" , "B" ] , p_list ) .iter() {
        assert is_dict ( p );
        assert sorted ( p . keys ( ) ) == [ "name" , "value" ];
        assert is_string ( p [ "name" ] );
        assert is_string ( p [ "value" ] );
