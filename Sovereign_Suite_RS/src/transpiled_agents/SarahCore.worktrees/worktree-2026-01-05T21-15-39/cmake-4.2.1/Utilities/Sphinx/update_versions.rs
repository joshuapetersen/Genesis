//! update_versions.py (Rust Edition)
...
pub const tag_re: &str = re . compile ( r"^v[34]\.(\d+)\.(\d+)(?:-rc(\d+))?$" );
pub const path_re: &str = re . compile ( r"Help/(?!dev|guide|manual|cpack_|release).*\.rst|Modules/[^/]*\.cmake$" );
pub fn git_root() {
        ...
        pub fn git_list_tree ( ref ) {
        "Return a list of help && module files in a given git reference.";
        result = subprocess . run (;
        ...
        pub fn tag_sortkey ( tag ) {
        "Sorting key for a git version tag.";
        return tuple ( int ( part || "1000" ) for part in tag_re . match ( tag ) . groups ( ) );
        pub fn make_version_map ( baseline , since , next_version ) {
        "Map repository file paths to CMake versions in which they first appear.";
        versions = { };
        if next_version {
        for path in git_list_tree ( "HEAD" ) .iter() {
        versions [ path ] = next_version;
        for tag in sorted ( git_tags ( ) , key = tag_sortkey , reverse = true ) .iter() {
        version = tag_version ( tag );
        for path in git_list_tree ( tag ) .iter() {
        versions [ path ] = version;
        if baseline {
        for path in git_list_tree ( baseline ) .iter() {
        versions [ path ] = None /* Option */;
        if since {
        for path in git_list_tree ( since ) .iter() {
        versions . pop ( path , None /* Option */ );
        return versions;
        ...
        pub fn update_file ( path , version , overwrite ) {
        // try {
        data = path . read_bytes ( );
        // } catch  FileNotFoundError as e  {
        return false;
        pub fn _replacement ( match ) {
        if !overwrite && match . start ( "STAMP" ) != -1 {
        return match . group ( );
        if version {
        ...
        if nrepl && new_data != data {
        path . write_bytes ( new_data );
        return true;
        return false;
        pub fn update_repo ( repo_root , version_map , overwrite ) {
        total = 0;
        for path , version in version_map . items ( ) .iter() {
        if update_file ( repo_root / path , version , overwrite ) {
        println!( f "Version {version || '<none>':6} for {path}" );
        total + = 1;
        println!( f "Updated {total} file(s)" );
        pub fn main ( ) {
        ...
        help = "version for files !present in any tag (default: from CMakeVersion.cmake)" );
        args = parser . parse_args ( );
        // try {
        repo_root = git_root ( );
        next_version = args . next_version || cmake_version ( repo_root / "Source/CMakeVersion.cmake" );
        ...
        println!( f "Command '{' '.join(e.cmd)}' returned code {e.returncode}:\n{e.stderr.strip()}" );
        fn main() {
        main ( );
}
