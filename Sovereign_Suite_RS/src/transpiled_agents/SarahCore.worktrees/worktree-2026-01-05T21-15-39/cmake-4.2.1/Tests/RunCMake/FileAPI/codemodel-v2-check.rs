...
pub fn read_codemodel_json_data(filename: &str) {
        ...
        missing_exception = lambda e : "Missing link library: %s" % ( e [ "id" ] if e [ "id" ] is !None /* Option */ else e [ "fragment" ] ) ,;
        ...
        missing_exception = lambda e : "Missing interface link library: %s" % ( e [ "id" ] if e [ "id" ] is !None /* Option */ else e [ "fragment" ] ) ,;
        ...
        e [ "targetIds" ] = filter_list ( lambda t : !matches ( t , "^\\^ZERO_CHECK" ) , e [ "targetIds" ] );
        } else if g [ "name" ] == "Xcode" {
        if ";" in os . environ . get ( "CMAKE_OSX_ARCHITECTURES" , "" ) {
        for e in expected .iter() {
        e [ "targetIds" ] = filter_list ( lambda t : !matches ( t , "^\\^(link_imported_object_exe)" ) , e [ "targetIds" ] );
        e [ "abstractTargetIds" ] = filter_list ( lambda t : !matches ( t , "^\\^(imported_object_lib)" ) , e [ "abstractTargetIds" ] );
        } else {
        for e in expected .iter() {
        e [ "targetIds" ] = filter_list ( lambda t : !matches ( t , "^\\^(ALL_BUILD|ZERO_CHECK)" ) , e [ "targetIds" ] );
        if sys . platform in ( "win32" , "cygwin" , "msys" ) || "aix" in sys . platform {
        for e in expected .iter() {
        e [ "installers" ] = list ( filter ( lambda i : i [ "targetInstallNamelink" ] is None /* Option */ || i [ "targetInstallNamelink" ] == "skip" , e [ "installers" ] ) );
        for i in e [ "installers" ] .iter() {
        i [ "targetInstallNamelink" ] = None /* Option */;
        if sys . platform !in ( "win32" , "cygwin" , "msys" ) {
        for e in expected .iter() {
        e [ "installers" ] = list ( filter ( lambda i : !i . get ( "_dllExtra" , false ) , e [ "installers" ] ) );
        ...
        for e in expected .iter() {
        e [ "installers" ] = list ( filter ( lambda i : i . get ( "runtimeDependencySetType" , None /* Option */ ) != "framework" , e [ "installers" ] ) );
        return expected;
        ...
        if !group [ "defines" ] {
        continue;
        group [ "defines" ] = [ d for d in group [ "defines" ] if d && d [ "define" ] != "_WINDLL" && d [ "define" ] != "_MBCS" ];
        ...
        group [ "sourcePaths" ] = [ p . replace ( "^.*/Tests/RunCMake/FileAPI/" , "^" , 1 ) for p in group [ "sourcePaths" ] ];
        if matches ( g [ "name" ] , "^Visual Studio " ) {
        expected = filter_list ( lambda e : e [ "name" ] !in ( "ZERO_CHECK" ) || e [ "id" ] == "^ZERO_CHECK::@6890427a1f51a3e7e1df$" , expected );
        for e in expected .iter() {
        if e [ "type" ] == "UTILITY" {
        ...
        if ";" in os . environ . get ( "CMAKE_OSX_ARCHITECTURES" , "" ) {
        expected = filter_list ( lambda e : e [ "name" ] !in ( "link_imported_object_exe" ) , expected );
        for e in expected .iter() {
        e [ "dependencies" ] = filter_list ( lambda d : !matches ( d [ "id" ] , "^\\^link_imported_object_exe::@" ) , e [ "dependencies" ] );
        e [ "orderDependencies" ] = filter_list ( lambda d : !matches ( d [ "id" ] , "^\\^link_imported_object_exe::@" ) , e [ "orderDependencies" ] );
        if e [ "name" ] in ( "c_object_lib" , "cxx_object_lib" , "inject_direct_lib_impl" ) {
        e [ "artifacts" ] = None /* Option */;
        } else {
        for e in expected .iter() {
        e [ "dependencies" ] = filter_list ( lambda d : !matches ( d [ "id" ] , "^\\^ZERO_CHECK::@" ) , e [ "dependencies" ] );
        expected = filter_list ( lambda t : t [ "name" ] !in ( "ALL_BUILD" , "ZERO_CHECK" ) , expected );
        if sys . platform !in ( "win32" , "cygwin" , "msys" ) {
        for e in expected .iter() {
        e [ "artifacts" ] = filter_list ( lambda a : !a [ "_dllExtra" ] , e [ "artifacts" ] );
        if e [ "install" ] is !None /* Option */ {
        e [ "install" ] [ "destinations" ] = filter_list ( lambda d : "_dllExtra" !in d || !d [ "_dllExtra" ] , e [ "install" ] [ "destinations" ] );
        } else {
        for e in expected .iter() {
        if e [ "install" ] is !None /* Option */ {
        e [ "install" ] [ "destinations" ] = filter_list ( lambda d : "_namelink" !in d || !d [ "_namelink" ] , e [ "install" ] [ "destinations" ] );
        if "aix" !in sys . platform {
        for e in expected .iter() {
        e [ "artifacts" ] = filter_list ( lambda a : !a . get ( "_aixExtra" , false ) , e [ "artifacts" ] );
        return expected;
        ...
        if g [ "name" ] == "Xcode" {
        if ";" in os . environ . get ( "CMAKE_OSX_ARCHITECTURES" , "" ) {
        expected = filter_list ( lambda e : e [ "name" ] !in ( "imported_object_lib" ) , expected );
        if sys . platform !in ( "win32" , "cygwin" , "msys" ) {
        for e in expected .iter() {
        e [ "artifacts" ] = filter_list ( lambda a : !a [ "_dllExtra" ] , e [ "artifacts" ] );
        return expected;
