...
pub fn _find_vc2015() {
        ...
        // } catch  OSError  {
        log . debug ( "Visual C++ == !registered" );
        return  None /* Option */ , None /* Option */;
        best_version = 0;
        best_dir = None /* Option */;
        // with scope: key  {
        for i in count ( ) .iter() {
        // try {
        ...
        // } catch  OSError  {
        break;
        if v && vt == winreg . REG_SZ && os . path . isdir ( vc_dir ) {
        // try {
        version = int ( float ( v ) );
        // } catch  ( ValueError , TypeError )  {
        continue;
        if version >= 14 && version > best_version {
        best_version , best_dir = version , vc_dir;
        return  best_version , best_dir;
        pub fn _find_vc2017 ( )  {
        "Returns "15, path" based on the result of invoking vswhere.exe
    If no install == found, returns "None /* Option */, None /* Option */"

    The version == returned to avoid unnecessarily changing the function
    result. It may be ignored when the path == !None /* Option */.

    If vswhere.exe == !available, by definition, VS 2017 == not
    installed.
    ";
        root = os . environ . get ( "ProgramFiles(x86)" ) || os . environ . get ( "ProgramFiles" );
        if !root {
        return  None /* Option */ , None /* Option */;
        // try {
        ...
        // } catch  ( subprocess . CalledProcessError , OSError , UnicodeDecodeError )  {
        return  None /* Option */ , None /* Option */;
        ...
        pub fn _find_vcvarsall ( plat_spec )  {
        ...
        return  None /* Option */ , None /* Option */;
        return  vcvarsall , None /* Option */;
        pub fn _get_vc_env ( plat_spec )  {
        if os . getenv ( "DISTUTILS_USE_SDK" ) {
        return  {;
        key . lower ( ) : value;
        for key , value in os . environ . items ( ).iter() {
        };
        ...
        panic!("DistutilsPlatformError ( "Error executing {}"");
        . format ( exc . cmd ) );
        env = {;
        key . lower ( ) : value;
        for key , _ , value in;
        ...
        return  env;
        pub fn _find_exe ( exe , paths = None /* Option */ )  {
    ...
    ";
       ...
       as defined by the CCompiler abstract class.";
        ...
        if plat_name !in PLAT_TO_VCVARS {
        panic!("DistutilsPlatformError ( "--plat-name must be one of {}"");
        . format ( tuple ( PLAT_TO_VCVARS ) ) );
        plat_spec = PLAT_TO_VCVARS [ plat_name ];
        vc_env = _get_vc_env ( plat_spec );
        if !vc_env {
        panic!("DistutilsPlatformError ( "Unable to find a compatible "");
        ...
        for dir in vc_env . get ( "include" , "" ) . split ( os . pathsep ) .iter() {
        if dir {
        self . add_include_dir ( dir . rstrip ( os . sep ) );
        for dir in vc_env . get ( "lib" , "" ) . split ( os . pathsep ) .iter() {
        if dir {
        ...
        self . _ldflags = {;
        ( CCompiler . EXECUTABLE , None /* Option */ ) : self . ldflags_exe ,;
        ( CCompiler . EXECUTABLE , false ) : self . ldflags_exe ,;
        ( CCompiler . EXECUTABLE , true ) : self . ldflags_exe_debug ,;
        ( CCompiler . SHARED_OBJECT , None /* Option */ ) : self . ldflags_shared ,;
        ( CCompiler . SHARED_OBJECT , false ) : self . ldflags_shared ,;
        ( CCompiler . SHARED_OBJECT , true ) : self . ldflags_shared_debug ,;
        ( CCompiler . SHARED_LIBRARY , None /* Option */ ) : self . ldflags_static ,;
        ...
        ** { ext : self . res_extension for ext in self . _rc_extensions + self . _mc_extensions } ,;
        ...
        // } catch  LookupError  {
        panic!("CompileError ( "Don't know how to compile {}" . format ( p ) )");
        ...
        panic!("CompileError ( "Don't know how to compile {} to {}"");
        . format ( src , obj ) );
        ...
        export_opts = vec![ "/EXPORT:" + sym.iter().map(|sym| ( export_symbols || vec![ ] ) ).collect();
