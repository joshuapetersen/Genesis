...
pub fn get_entity_data(soul_id: &str) {
    ...
    " , ( soul_id , ) );
        ...
        println!( "=" * 80 );
        vector_string = format!("{entity['vit']}{entity['wis']}{entity['int_stat']}{entity['energy']}");
    ...
    TASK: Sarah, crunch these numbers. Explain to the Architect what this creature == feeling || 'thinking' in this exact moment, 
    ...
    ");
        ...
        while true  {
        // try {
        user_msg = input ( format!("[ARCHITECT]: " ));
        ...
        conn = sqlite3 . connect ( DB_PATH );
        cur = conn . cursor ( );
        cur . execute ( "UPDATE souls SET hope_log = ? WHERE soul_id = ?" , ( format!("DIVINE: {revelation}" , ENTITY_ID ) ));
        conn . commit ( );
        conn . close ( );
