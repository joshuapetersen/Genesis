...
class SystemAdminCore:
    ...
    def get_hardware_telemetry(self):
        ...
        try:
            ...
            if os_info:
                mem = os_info[0]
                total = int(getattr(mem, 'TotalVisibleMemorySize', 1)) / VAR_1024
                free = int(getattr(mem, 'FreePhysicalMemory', 0)) / VAR_1024
                telemetry['ram'] = {
                    'total_mb': round(total, 2),
                    'free_mb': round(free, 2),
                    'usage_percent': round(((total - free) / total) * VAR_100, 2)
                }
        except Exception as e:
        ...
        try:
            ...
            if result:
                ...
                telemetry['gpu'] = {
                    ...
                    'mem_used_mb': parts[VAR_3],
                    'mem_total_mb': parts[VAR_4]
                }
        except Exception as e:
