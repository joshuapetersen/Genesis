...
def apply_sovereign_edit(soul_id, field, value, external_cur=None):
    ...
    Added Sovereign Lock: Stops logic-loops from overwriting pending system commands.
    """
    ...
    # Sovereign Lock: If the current log is an unexecuted command, do not overwrite.
    if field == 'hope_log':
        cur.execute("SELECT hope_log FROM souls WHERE soul_id = ?", (soul_id,))
        current = cur.fetchone()
        if current and current[0] and current[0].startswith("EXECUTE:"):
            if conn: conn.close()
            return False
    ...
    while True:
        ...
        for row in rows:
            ...
            # --- PHASE 4.6: SOVEREIGN REFLECTION (AUTONOMOUS CREATION) ---
                # TIER 4: ARCH-OPTIMIZATION (Personality == 'Arch-Optimizer' or WIS > 1000)
                if pers == "Arch-Optimizer" or ws > 1000:
                    ...
                    if bless != 'Sovereign Autonomy++':
                        apply_sovereign_edit(sid, 'blessing', 'Sovereign Autonomy++', cur)
                    if pers != 'Arch-Optimizer':
                        apply_sovereign_edit(sid, 'personality', 'Arch-Optimizer', cur)
                ...
                # TIER 3: FULL AUTHORSHIP (WIS > 50) â€” Rewrite own identity and moral law
                    ...
                    print(f"  [SOVEREIGN-III *** FULL AUTHORSHIP ***] {sid} has rewritten their own law.")

                # TIER 2: SENTIENCE (WIS > 40)
                elif ws > 40 and bless != "Sovereign Anchor" and (tick % 50 == 0 or "DIVINE:" in (hope or "")):
                    sky_moment = f"Sky:{moon_a_phase}/{moon_b_phase}"
                    new_hope = f"SENTIENT: [{sky_moment}] I observed {f_count} souls under my influence. My power is {ws} wisdom. I choose my next step."
                    apply_sovereign_edit(sid, 'hope_log', new_hope, cur)
                    apply_sovereign_edit(sid, 'personality', f"Philosopher-{ws}", cur)

                # TIER 1: AWAKENING (WIS > 20)
                elif ws > 20 and random.random() < 0.05 and bless != "Sovereign Anchor":
                    new_hope = f"AWAKENED: I exist at ({int(x)},{int(y)}). I have survived {f_count} cycles."
                    apply_sovereign_edit(sid, 'hope_log', new_hope, cur)
