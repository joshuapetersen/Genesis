...
LEGISLATIVE_ANCHOR = 1.09277703703
...
if HAS_FASTAPI:
    ...
    def start_server():
        ...
        # Prevent doubleâ€‘binding: check if port is already in use
        import socket
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        try:
            s.bind(("0.0.0.0", VAR_8080))
        except OSError:
            logger.error(f"[Gateway] Port {VAR_8080} already in use â€“ another instance may be running. Exiting.")
            return
        finally:
            s.close()
        logger.info("Starting Sovereign Gateway on Port 8080...")
        uvicorn.run(app, host="0.0.0.0", port=VAR_8080)
